...
@if ($current)
  <div
    {{ $attributes->class([
        'px-3 py-2 rounded-lg mb-3 flex items-center gap-2 border text-sm',
        $current['container_class'],
    ]) }}>
    <i class="{{ $current['icon'] }} text-sm shrink-0"></i>
    <span class="font-medium">{{ str_replace(':pembimbing', $pembimbingLabel, $current['title']) }}</span>
  </div>
@endif
