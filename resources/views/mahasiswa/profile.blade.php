      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          ...
          <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Peminatan</label>
              @php
                $peminatanLabel = ['RPL' => 'Rekayasa Perangkat Lunak', 'KCV' => 'Komputasi Cerdas dan Visual', 'KBJ' => 'Komputasi Berbasis Jaringan'][$profile->peminatan] ?? $profile->peminatan ?? '-';
              @endphp
              <input type="text" value="{{ $peminatanLabel }}" disabled
                class="w-full rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-500 cursor-not-allowed">
              <p class="mt-1 text-xs text-slate-400">Peminatan tidak dapat diubah.</p>
            </div>

            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Angkatan</label>
              <input type="text" value="{{ $profile->angkatan }}" disabled
                class="w-full rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-500 cursor-not-allowed">
              <p class="mt-1 text-xs text-slate-400">Angkatan tidak dapat diubah.</p>
            </div>
