...
@section('content')
  ...
  <div class="bg-gradient-to-br from-amber-600 to-amber-400 rounded-2xl p-5 sm:p-8 mb-6 text-white">
    <div>
      <h1 class="text-xl sm:text-2xl font-bold mb-0.5"><i class="fas fa-calendar-plus mr-2"></i>Edit Periode Akademik</h1>
        ...
        <div class="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input type="text" name="tahun_ajaran" value="{{ old('tahun_ajaran', $periode->tahun_ajaran) }}"
              placeholder="Contoh: 2025/2026" @if ($periode->status !== 'draft') disabled @endif required
              class="w-full px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all
                @error('tahun_ajaran') border-red-400 bg-red-50 @else border-gray-300 @enderror
                {{ $periode->status !== 'draft' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-800 focus:border-blue-500' }}" />
            @error('tahun_ajaran')
          ...
          <div>
            ...
            <select name="semester" @if ($periode->status !== 'draft') disabled @endif required
              class="w-full pl-3 pr-8 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/10 transition-all
                @error('semester') border-red-400 bg-red-50 @else border-gray-300 @enderror
                {{ $periode->status !== 'draft' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-800 focus:border-blue-500' }}">
              <option value="">-- Pilih Semester --</option>
              <option value="ganjil" {{ old('semester', $periode->semester) === 'ganjil' ? 'selected' : '' }}>Ganjil
              </option>
              <option value="genap" {{ old('semester', $periode->semester) === 'genap' ? 'selected' : '' }}>Genap
              </option>
            </select>
            @error('semester')
              <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
          ...
          <div>
            ...
            <input type="date" name="selesai_at"
