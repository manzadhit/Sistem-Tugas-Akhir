...
@section('title', 'Edit Mahasiswa - ' . $mhs->nama_lengkap)
...
@section('content')
  ...
  <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
    <a href="{{ route('admin.mahasiswa.index') }}" class="hover:text-blue-600 transition-colors">Kelola Mahasiswa</a>
    <i class="fas fa-chevron-right text-xs text-gray-400"></i>
    <a href="{{ route('admin.mahasiswa.show', $mhs->id) }}"
      class="hover:text-blue-600 transition-colors truncate max-w-[160px]">{{ $mhs->nama_lengkap }}</a>
    <i class="fas fa-chevron-right text-xs text-gray-400"></i>
  ...
  <form method="POST" action="{{ route('admin.mahasiswa.update', $mhs->id) }}">
    @csrf
    @method('PUT')
      ...
      <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div class="sm:col-span-2">
            ...
            <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $mhs->nama_lengkap) }}" required
          ...
          {{-- NIM (readonly) --}}
          <div>
            <label class="block text-xs font-medium text-gray-600 mb-1.5">NIM</label>
            <div class="relative">
            ...
            <p class="text-xs text-gray-400 mt-1">NIM tidak dapat diubah</p>
          </div>
          ...
          <div>
            ...
            <input type="number" name="angkatan" value="{{ old('angkatan', $mhs->angkatan) }}" required min="2000"
              max="{{ date('Y') }}"
          ...
          <div>
            ...
            <input type="text" name="jurusan" value="{{ old('jurusan', $mhs->jurusan) }}" required
          ...
          <div>
            ...
            <input type="text" name="program_studi" value="{{ old('program_studi', $mhs->program_studi) }}" required
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="text" name="no_telp" value="{{ old('no_telp', $mhs->no_telp) }}" placeholder="08xxxxxxxxxx"
          ...
          <div>
            ...
            <select name="status_akademik" required
              ...
              @foreach (['aktif' => 'Aktif', 'cuti' => 'Cuti', 'nonaktif' => 'Non-aktif', 'lulus' => 'Lulus', 'dropout' => 'Dropout'] as $val => $lbl)
                <option value="{{ $val }}"
                  {{ old('status_akademik', $mhs->status_akademik) === $val ? 'selected' : '' }}>
                  {{ $lbl }}
                </option>
              @endforeach
            </select>
  ...
  <div x-data="{ open: false }" x-on:open-delete-modal.window="open = true" x-show="open" x-transition.opacity
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
      ...
      <p class="text-sm text-gray-600 mb-5">
        ...
        <span class="font-semibold text-gray-900">{{ $mhs->nama_lengkap }}</span>
        (NIM: <span class="font-semibold">{{ $mhs->nim }}</span>)?
      ...
      <div class="flex gap-3">
        ...
        <form method="POST" action="{{ route('admin.mahasiswa.destroy', $mhs->id) }}" class="flex-1">
          @csrf
          @method('DELETE')
          <button type="submit"
