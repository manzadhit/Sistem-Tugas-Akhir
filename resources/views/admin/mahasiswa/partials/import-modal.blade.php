...
<x-modal name="import-mahasiswa" :show="$showImportErrors" maxWidth="lg" focusable>
  <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-3">
    <div>
      <h2 class="text-lg font-semibold text-gray-900">Import Data Mahasiswa</h2>
      <p class="text-sm text-gray-500">Unggah file CSV, XLS, atau XLSX sesuai format yang ditentukan.</p>
    </div>
    <button type="button" x-on:click="$dispatch('close-modal', 'import-mahasiswa')"
  ...
  <form method="POST" action="{{ route('admin.mahasiswa.import') }}" enctype="multipart/form-data">
    ...
    <div class="px-6 py-5 space-y-4">
      ...
      <div class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        ...
        <strong>nim</strong>, <strong>nama lengkap</strong>.
        <br>
        <span class="text-xs text-blue-600">Angkatan dihitung otomatis dari NIM (pola E1E1YY...). Jurusan default: Informatika.</span>
      </div>
