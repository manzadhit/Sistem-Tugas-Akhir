...
<x-modal name="import-mata-kuliah" :show="$showImportErrors" maxWidth="lg" focusable>
  <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-3">
    <div>
      <h2 class="text-lg font-semibold text-gray-900">Import Mata Kuliah</h2>
      <p class="text-sm text-gray-500">Unggah file CSV, XLS, atau XLSX sesuai template yang sudah disediakan.</p>
    </div>
    <button type="button" x-on:click="$dispatch('close-modal', 'import-mata-kuliah')"
      class="w-9 h-9 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors">
      <i class="fas fa-times text-sm"></i>
    </button>
  </div>

  <form method="POST" action="{{ route('admin.mata-kuliah.import') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="px-6 py-5 space-y-4">
      ...
      <div class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        Gunakan file dengan kolom <strong>kode</strong> dan <strong>nama</strong>.
        Anda bisa mengunduh template di tombol <strong>Unduh Template</strong>.
      </div>
