...
@section('content')
  ...
  <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
    <a href="{{ route('admin.publikasi.index') }}" class="hover:text-blue-600 transition-colors">Kelola Publikasi</a>
    <i class="fas fa-chevron-right text-xs text-gray-400"></i>
    <a href="{{ route('admin.publikasi.show', $publikasi->id) }}"
  ...
  <form method="POST" action="{{ route('admin.publikasi.update', $publikasi->id) }}">
    @csrf
    @method('PUT')
        ...
        <div class="px-5 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div class="sm:col-span-2">
            ...
            <select name="dosen_id" required
              class="w-full pl-3 pr-8 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-800 bg-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all @error('dosen_id') border-red-400 bg-red-50 @enderror">
              <option value="">-- Pilih Dosen --</option>
              @foreach ($daftarDosen as $dosen)
                <option value="{{ $dosen->id }}"
                  {{ old('dosen_id', $publikasi->dosen_id) == $dosen->id ? 'selected' : '' }}>
                  {{ $dosen->nama_lengkap }}
                </option>
              @endforeach
            </select>
  ...
  <div x-data="{ open: false }" x-on:open-delete-modal.window="open = true" x-show="open" x-transition.opacity
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
      ...
      <div class="flex gap-3">
        ...
        <form method="POST" action="{{ route('admin.publikasi.destroy', $publikasi->id) }}" class="flex-1">
          @csrf
          @method('DELETE')
          <button type="submit"
