...
@section('content')
  ...
  <div class="relative h-32 sm:h-40 overflow-hidden rounded-xl bg-gradient-to-br from-blue-800 to-blue-500 mb-6 sm:mb-8">
    <div class="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-4">
      <h1 class="text-xl sm:text-2xl font-bold mb-1">Permintaan Dosen Pembimbing</h1>
      <p class="text-xs sm:text-sm opacity-80">Tinjau pengajuan mahasiswa dan lanjutkan proses penetapan dosen pembimbing.</p>
    </div>
  </div>
  ...
  <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="flex flex-col gap-4 border-b border-slate-200 p-4 sm:p-5 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="mt-0.5 sm:mt-1 text-xs sm:text-sm text-slate-500">Mahasiswa yang mengajukan permintaan dosen pembimbing tugas akhir</p>
      </div>
      <form action="{{ route('kajur.permintaan-pembimbing') }}" method="GET"
      ...
      @forelse ($permintaanPembimbing as $index => $permintaan)
        <div class="border-b border-slate-100 p-4 last:border-b-0">
          ...
          <div class="flex items-center justify-between mb-2.5">
            <span class="text-xs font-medium text-slate-400">#{{ $permintaanPembimbing->firstItem() + $index }}</span>
          ...
          {{-- Mahasiswa info --}}
          <div class="mb-2">
            <p class="font-semibold text-sm text-slate-800">{{ $permintaan->mahasiswa->nama_lengkap }}</p>
            <p class="text-xs text-slate-500">NIM: {{ $permintaan->mahasiswa->nim }}</p>
          </div>

          {{-- Judul TA --}}
          <div class="mb-3">
            <p class="text-xs font-medium text-slate-400 mb-0.5">Judul Tugas Akhir</p>
            <p class="text-sm text-slate-600 leading-relaxed">{{ $permintaan->judul_ta }}</p>
          </div>
