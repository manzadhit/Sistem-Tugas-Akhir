...
<x-modal name="import-publikasi-dosen" :show="$showImportErrors" maxWidth="lg" focusable>
  <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between gap-3">
    <div>
      <h2 class="text-lg font-semibold text-gray-900">Import Publikasi</h2>
      <p class="text-sm text-gray-500">Unggah file CSV, XLS, atau XLSX sesuai template publikasi dosen.</p>
    </div>
    <button type="button" x-on:click="$dispatch('close-modal', 'import-publikasi-dosen')"
      class="w-9 h-9 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors">
      <i class="fas fa-times text-sm"></i>
    </button>
  </div>

  <form method="POST" action="{{ route('dosen.publikasi.import') }}" enctype="multipart/form-data">
    @csrf
