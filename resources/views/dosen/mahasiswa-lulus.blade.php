...
@section('title', 'Daftar Mahasiswa Lulus')
...
@section('content')
  ...
  <div class="mb-8">
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-3">
      ...
      <span class="text-gray-900 font-medium">Mahasiswa Lulus</span>
    </nav>
    <div class="flex items-center gap-3 mb-4">
      <a href="{{ route('dosen.bimbingan.index') }}" class="text-gray-500 hover:text-gray-700 transition-colors">
        <i class="fas fa-arrow-left"></i>
      </a>
      <h1 class="text-3xl font-bold text-gray-900">Daftar Mahasiswa Lulus</h1>
    </div>
    <p class="text-base text-gray-500">Mahasiswa bimbingan Anda yang telah menyelesaikan studi</p>
  </div>

  <!-- Search Filter -->
  <div class="bg-white rounded-xl p-4 sm:p-6 shadow-sm mb-6">
    <form method="GET" action="{{ route('dosen.bimbingan.mahasiswa-lulus') }}" id="filterForm">
  ...
  <div class="space-y-3 lg:hidden">
    @forelse ($mahasiswaLulus as $dp)
      <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
        <div class="flex items-start justify-between gap-3 mb-3">
          ...
          <span class="inline-block px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 shrink-0">
            Lulus
          </span>
        </div>
        <div class="space-y-1.5 text-xs sm:text-sm">
          ...
          <div>
            <div class="text-xs text-gray-400">Peran</div>
            <div class="text-gray-700">{{ $dp->getJenisPembimbing() }}</div>
          </div>
        </div>
    ...
    @empty
      <div
        ...
        Belum ada mahasiswa bimbingan yang lulus.
      </div>
    @endforelse
  </div>
  ...
  <div class="hidden lg:block bg-white rounded-xl shadow-sm overflow-x-auto">
    <table class="w-full min-w-[720px] border-collapse">
      ...
      <tbody>
        @forelse ($mahasiswaLulus as $dp)
          <tr class="hover:bg-gray-50 transition-colors">
            <td class="p-4 text-sm text-gray-500 border-b border-gray-100">
              {{ $mahasiswaLulus->firstItem() + $loop->index }}</td>
        ...
        @empty
          <tr>
            <td colspan="6" class="p-6 text-sm text-gray-500 text-center">Belum ada mahasiswa bimbingan yang lulus.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
  ...
  <div class="mt-4">
    {{ $mahasiswaLulus->links('pagination::tailwind') }}
  </div>
@endsection
